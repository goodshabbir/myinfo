<div class="row">
    <div class="col-sm-12">
        <div class="card-box">
            <h4 class="m-t-0 header-title">Direct Income</h4>
        </div>
    </div>
</div>
<?php if(!empty($directHistory)) { ?>
<div class="row">
    <div class="col-sm-12">
        <div class="card-box table-responsive">
            <h4 class="m-t-0 header-title"><b>Direct Sponsor Income History</b></h4>
            ...
            <table id="datatable" class="table table-striped table-bordered dt-responsive nowrap" cellspacing="0"
                   ...
                   <tbody>
                   <?php $i=1; foreach($directHistory as $dir) : ?>
                        <tr>
                            <td><?= $i++ ?></td>
                            <td><?= date('M d, Y',strtotime($dir->on_closing)); ?></td>
                            <td><?= $dir->amount; ?></td>
                            <td><?= $dir->duration; ?></td>
                            <td><?= $dir->elapse_time; ?></td>
                            <td><?= $dir->remaning_time; ?></td>
                            <td><a href="javascript:;" onclick="showDirectIncomeHistory('<?= $dir->sponsor_id ?>', '<?= $dir->id ?>')">Click Here</a></td>
                        </tr>
                    <?php endforeach; ?>
                   </tbody>
            </table>
...
<div class="row">
    <div class="col-sm-12">
        <div class="card-box table-responsive">
            <h4 class="m-t-0 header-title"><b>Direct Income</b></h4>
            <p class="text-muted font-13 m-b-30">
            ...
            <table id="datatable" class="table table-striped table-bordered dt-responsive nowrap" cellspacing="0"
                ...
                <tbody>
                  <?php $i=1; foreach($directIncome as $row): ?>
                  <tr>
                      <td><?= $i++; ?></td>
                      ...
                      <td><?php if($row->status==0){ echo "<span class='label label-warning'>Yet not released</span>"; } else{ echo  date('M d, Y', strtotime($row->paid_date)); } ?></td>
...
function showDirectIncomeHistory(spoID, id){
   
    $.ajax({
        url:'<?= site_url('directhistory'); ?>',
        type: 'POST',
        dataType: 'json',
        data:{sponsor_id:spoID, id:id },
        success: function(result){
            $("#resultt").html(result.table);
        },
        error:function(result){
            $.Notification.notify('error','top-right','Opps! something went wrong please try to another time','Sorry for inconviniance we are working continue for this please be patience , we give you better');
        }
    });
    $("#custom-width-modal1").modal('show');
}
</script>